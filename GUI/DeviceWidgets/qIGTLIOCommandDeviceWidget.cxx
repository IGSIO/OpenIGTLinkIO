...
{
  igtlioCommandDevice* device = dynamic_cast<igtlioCommandDevice*>(Device.GetPointer());
  if (!device)
    return;

  igtlioCommandConverter::ContentData content = device->GetContent();
...
void qIGTLIOCommandDeviceWidget::onDeviceModified()
{
  ...
  std::string device_content = device->GetContent().content;
  if(device_content != ContentEdit->toPlainText().toStdString())
    ContentEdit->setText(device_content.c_str());
